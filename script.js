...
function loadFavorites() {
    try {
        ...
        // 過去のメモデータにtext, categoryId, folderが欠けている場合に補完するロジック
        return loadedFavorites.map(fav => {
            if (!fav.categoryId || !fav.folder || !fav.text) {
                // フォルダ名、カテゴリID、テキストをVOICE_DATAから探して補完する
                for (const category of VOICE_DATA) {
                    ...
                    if (foundVoice) {
                        ...
                        fav.text = foundVoice.text; // テキストも補完
                        break;
                    }
                }
            }
